...
function Login2(){
    ...
    const handleSignup = async (event) => {
      ...
      console.log(event.target); // Check what's actually in the event target

      const formData = new FormData(event.target); // Use FormData API
    const data = {
        name: formData.get('first_name'), // Ensure 'name' attributes match these strings
        email: formData.get('email'),
        contact: formData.get('password'), // This seems wrong; make sure this should not be 'contact'
    ...
    try {
        ...
        if (response.ok) {
            ...
            navigate("/dashboard6"); // Navigate to home after successful signup
        } else {
